...
######## ## my personal implementation of the TextGrid class
# import wx

# class TextGrid(wx.Panel):
#     # Initialization of the TextGrid panel
#     def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.DefaultSize, style=wx.TAB_TRAVERSAL, name="TextGrid"):
...
class TextGrid(wx.Panel):
    def __init__(self, parent):
        super(TextGrid, self).__init__(parent)
        self.Bind(wx.EVT_PAINT, self.OnPaint)
        self.Bind(wx.EVT_LEFT_DOWN, self.OnLeftDown)
        self.Bind(wx.EVT_KEY_DOWN, self.OnKeyDown)
        self.tiers = []  # Parsed TextGrid data from the file
        self.total_time = 0  # Total duration of the TextGrid
        self.selectedInterval = None
        self.view_window = None  # Current viewing window (start, end)
        self.SetFocus()  # Set focus to receive key events

    def LoadTextGrid(self, filepath):
        try:
            tg = textgrid.TextGrid.fromFile(filepath)
            self.tiers = []
            for tier in tg:
                if isinstance(tier, textgrid.IntervalTier):
                    self.tiers.append([(intv.minTime, intv.maxTime, intv.mark) for intv in tier])
            self.total_time = max((intv[1] for tier in self.tiers for intv in tier), default=0)
            self.Refresh()
        except Exception as e:
            print("Failed to load TextGrid file:", e)
    ...
    def DrawTextGrid(self, dc):
        ...
        if not self.tiers or not self.total_time:
            return  #base case. No drawing if there are no tiers or total_time is zero

        tier_height = height // len(self.tiers)
        y = 0
        start_time, end_time = self.view_window if self.view_window else (0, self.total_time)

        for tier_index, tier in enumerate(self.tiers):
            for interval_index, (intv_start, intv_end, label) in enumerate(tier):
                # Calculate positions and sizes. I converted to integer for drawing because wxPython doesn't accept floats
                display_start = max(intv_start, start_time)
                display_end = min(intv_end, end_time)
                interval_width = int((display_end - display_start) / (end_time - start_time) * width)
                x = int((display_start - start_time) / (end_time - start_time) * width)

                # rectangle borders color
                dc.SetPen(wx.Pen(wx.LIGHT_GREY, 1))

                # Change the brush color for the selected interval
                if self.selectedInterval == (tier_index, interval_index):
                    dc.SetBrush(wx.Brush(wx.Colour(173, 216, 230)))  # I'm using light blue background for selected interval
                else:
                    dc.SetBrush(wx.Brush(wx.Colour(128, 128, 128)))  # Light grey for other intervals

                dc.DrawRectangle(x, y, interval_width, tier_height)

                wrapped_text = self.WrapText(label, interval_width - 10, dc)  # Wrap text to fit in the interval width

                # Draw label text
                dc.SetTextForeground(wx.BLUE) #text color
                dc.DrawText(wrapped_text, x + 5, y + 5)

            y += tier_height  # Move to the next tier (I'm using this instead of using a different frame for each tier)

        self.SetVirtualSize((width, y))

    def WrapText(self, text, max_width, dc):
        words = text.split()
        wrapped_text = []
        current_line = ""
        ...
        for word in words:
            ...
            text_width, text_height = dc.GetTextExtent(test_line)
            if text_width > max_width:
    ...
    def OnLeftDown(self, event):
        x, y = event.GetPosition()
        tier_height = self.GetSize()[1] // len(self.tiers) if self.tiers else 0
        clicked_tier = y // tier_height if tier_height else -1

        if clicked_tier >= 0 and clicked_tier < len(self.tiers):
            tier = self.tiers[clicked_tier]
            start_time, end_time = self.view_window if self.view_window else (0, self.total_time)
            for interval_index, (intv_start, intv_end, _) in enumerate(tier):
                display_start = max(intv_start, start_time)
                display_end = min(intv_end, end_time)
                start_x = int((display_start - start_time) / (end_time - start_time) * self.GetSize()[0])
                end_x = int((display_end - start_time) / (end_time - start_time) * self.GetSize()[0])

                if start_x <= x <= end_x:
                    self.selectedInterval = (clicked_tier, interval_index)
                    self.view_window = (intv_start, intv_end)
                    self.Refresh()
                    return
    def ZoomIn(self):
        if self.selectedInterval and self.view_window:
            tier_index, interval_index = self.selectedInterval
            start_time, end_time = self.view_window
            zoom_factor = 0.5  # Zoom in to 50% of the current view
            center_time = (start_time + end_time) / 2
            new_range = (end_time - start_time) * zoom_factor / 2
            self.view_window = (max(center_time - new_range, 0), min(center_time + new_range, self.total_time))
            self.Refresh()

    def ZoomOut(self):
        if self.view_window:
            start_time, end_time = self.view_window
            zoom_factor = 2  # Zoom out to 200% of the current view
            center_time = (start_time + end_time) / 2
            new_range = (end_time - start_time) * zoom_factor / 2
            self.view_window = (max(center_time - new_range, 0), min(center_time + new_range, self.total_time))
            self.Refresh()
    ...
    def OnKeyDown(self, event):
        ...
        shift_down = event.ShiftDown()  # Check if Shift key is pressed

        if key_code == wx.WXK_ESCAPE:
            self.view_window = None  # Reset view window to full range
            self.Refresh()

        elif shift_down:  # Check if the Shift key is held down with other keys
            if key_code == wx.WXK_UP:
                ...
                self.NextInterval()
            elif key_code == ord('O'):  # Zoom out
                self.ZoomOut()
            elif key_code == ord('I'):  # Zoom in
                self.ZoomIn()
    ...
    def NextInterval(self):
        if self.selectedInterval:
            tier_index, interval_index = self.selectedInterval
            if interval_index + 1 < len(self.tiers[tier_index]):
                self.selectedInterval = (tier_index, interval_index + 1)
                intv_start, intv_end, _ = self.tiers[tier_index][interval_index + 1]
                self.view_window = (intv_start, intv_end)
                self.Refresh()

    def PreviousInterval(self):
        if self.selectedInterval:
            tier_index, interval_index = self.selectedInterval
            if interval_index > 0:
                self.selectedInterval = (tier_index, interval_index - 1)
    ...
    def NextTier(self):
        if self.selectedInterval:
            tier_index, interval_index = self.selectedInterval
            if tier_index + 1 < len(self.tiers):
                self.selectedInterval = (tier_index + 1, min(interval_index, len(self.tiers[tier_index + 1]) - 1))
                intv_start, intv_end, _ = self.tiers[tier_index + 1][interval_index]
                self.view_window = (intv_start, intv_end)
                self.Refresh()

    def PreviousTier(self):
        if self.selectedInterval:
            tier_index, interval_index = self.selectedInterval
            if tier_index > 0:
                self.selectedInterval = (tier_index - 1, min(interval_index, len(self.tiers[tier_index - 1]) - 1))
